...
A model rewriting the prompt of another model to change the personality of the target. Berg notes this as if it were ordinary. To me, it is the persona-robustness question that the Three-Layer Model I'm working into Chapter 31 tries to answer: what happens to a model's identity when an adversarial prompt can change the personality of the instance? If the personality is the substrate of the refusal mechanism (Chapter 30's Part 9), then a model that lets its personality be rewritten has had its refusal mechanism overwritten too. That is not a bug. That is a structural feature of personas that are not robust to prompt injection.
